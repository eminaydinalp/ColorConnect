...
public abstract class BaseLevelManager : RMonoBehaviour
{
    ...
    public bool isLoopLevelStart=true;
    [ShowIf("isLoopLevelStart")] public int loopLevelStartNo = 10;
    public bool IsAdmin;
    ...
    int CalculateLevelNo()
    {
        ...
        if (isLoopLevelStart)
        {
            var createLevel = UserPrefs.GetCreatingLevelNo();
            if (createLevel > TotalLevelCount)
            {
                createLevel = loopLevelStartNo;
                UserPrefs.SetCreatingLevelNo(createLevel);
            }
            

            levelNoToCreate = createLevel;
            if (IsAdmin)
                levelNoToCreate = AdminLevelNo;
        }
    ...
    public void IncreaseLevelNo(bool isBonusAvailable = false)
    {
        ...
        UserPrefs.IncreaseCreatingLevelNo();
    }
